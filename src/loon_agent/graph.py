...
def _build_messages(
    ...
    """Assemble the message list for the model: system + memory context + history."""
...
def build_graph(
    ...
    *,
    ...
    def agent_node(state: AgentState, config: RunnableConfig) -> dict[str, list[BaseMessage]]:
        messages = _build_messages(state, config, memory)
...
class LoonAgent:
    ...
    def __init__(
        ...
        *,
        ...
        self.memory = memory
        self.graph = build_graph(llm, tools, checkpointer=checkpointer, memory=memory)
